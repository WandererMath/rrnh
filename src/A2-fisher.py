...
def to_fisher(Y, N):
    return [[int(N1*a),int(N1*(1-a))] for a in Y]
...
for j in range(len(letters)):
    ...
    Y1_t=to_fisher(Y1, N1)
    Y2_t=to_fisher(Y2, N2)
    Ref_t=to_fisher(Ref, N3)
    ...
    for i in range(len(Y1_t)):
        ...
        _, p1 = stats.fisher_exact([Ref_t[i], Y1_t[i]])
        _, p2 = stats.fisher_exact([Ref_t[i], Y2_t[i]])
        T1.append(p1)
        T2.append(p2)
    plt.plot(X, T1, label='Stimulated')
    plt.plot(X, T2, label="Reduced")
    plt.ylabel("p-value in log scale")
    plt.yscale('log')
    plt.ylim(0.01, 1)
    ...
    plt.savefig(letters[j]+"-fisher.pdf")
    plt.cla()
