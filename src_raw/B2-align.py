...
INPUT=get_files("../data07/dedup")
INPUT_BASE=basenames(INPUT)
OUT_PATH="../data07/bowtie-d"
os.makedirs(OUT_PATH, exist_ok=True)
ref_genome_loc ="../ref/Ecoli_BW25113"
print(INPUT)
print(INPUT_BASE)
def align():
    with open("B2.sh", 'w') as f:
        f.write(SLURM)
        for file, out in zip(INPUT, INPUT_BASE):
            ...
            command="bowtie -q -3 5 -S -k 10 {} {} > {}".format(ref_genome_loc,file,os.path.join(OUT_PATH, out+".txt"))
            f.write(command+" & \n")
        f.write("wait\n")
